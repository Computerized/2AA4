...
\item Programming levels perform abstractions on various levels. For example, with Object Oriented Programming Languages, an entire program can be abstracted into a data type (some kind of object), which contains various operations and sets of data. Programming languages also use functions/methods, which abstract a set of instructions into a single function/method call. The simplest operations (such as "x = 1 + 2") represent abstractions of data manipulation on the hardware level %fix this
