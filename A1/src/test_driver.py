...
passed = real_test() + imag_test() + get_r_test() + get_phi_test() + equal_test() + conj_test() + add_test() + sub_test()
print("complex_adt: Passed " + str(passed) + " tests out of 8")
...
def tri_type_test():
    if ((tr1.tri_type() == TriType.right) and (tr2.tri_type() == TriType.equilat) and (tr6.tri_type() == TriType.isosceles) and (tr4.tri_type() == TriType.scalene)):
        print("Passed tri_type() test")
        return 1
    else:
        print("Failed tri_type() test")
        return 0
