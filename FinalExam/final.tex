...
\item ALOC/day encourages the overabundance of code, which can often lead to unnecessary and/or needlessly complicated code.
\item ALOC/day can vary depending on the program or the language (ie Java programs will normally lead to more lines than Python code due to the differnce in method inclusion, since Java programs ofthen use an extra line for a '\{' character).
\item ALOC/day does not take into account the actual content of the code, as coding a more complex algorithm may take fewer lines than coding something basic (for example a 30 line algorithm will likely take more time to code than 50 lines of System.out.println()).
\end{itemize}
...
\begin{itemize}
\item \textbf{Abstract} - The specification specifically refers to a given app, rather than referring to an app abstractly.
\item \textbf{Unambiguous} - The specification is very ambiguous, as the term "easy to use" is not well defined.
\item \textbf{Validatable} - The specification is not easily validatable, as there is no given metric with which ease of use is to be measured.
...
The process of verification can be verified with a VnV report, which will verify that our verification is indeed verifying correct behaviour.
