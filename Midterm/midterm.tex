...
NAME: \wss{Alan Scott}\\[1ex]

Student ID: \wss{400263658} \\[2mm]
...
\begin{enumerate}[a.]
\item Currently, the module operates over $\mathbb{Z}$. To make this module more generic, you could change the value type to be of a generic type, which could then be inputted through a parameter in the constructor, which the operations could then range over. 
\item To make the specification generic to Type T, the specification would need to specify the type T and Sequence of T (in place of $\mathbb{Z}$) for the Exported Access Programs, as well as making the state variable \texttt{s} of type T. The type ranges for the constructor, \texttt{union, equals, set\_to\_seq} and \texttt{complement} would have to be changed from $\mathbb{Z}$ to T. Complement would also need to be modified to work over the range of every element in T instead of $\mathbb{Z}$.
\item In order for T to be a valid type, there must be a defined relationship for equivalence between two elements in T in order to have some basis of comparison between elements of the type. 
\item (BONUS) \\
t\_equals: T $\times$ T \\
t\_equals($t_1,t_2$) $\equiv (t_1 = t_2)$
\end{enumerate}
